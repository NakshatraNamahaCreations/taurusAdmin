...
function Pendingpaymentreport() {
  ...
  const handleFilter = (type) => {
    ...
    switch (type) {
      ...
      case "week":
        filtered = pendingPayments.filter((p) =>
          p._nextPaymentDateObj?.isBetween(today.startOf("week"), today.endOf("week"), null, "[]")
        );
        break;
      case "month":
        filtered = pendingPayments.filter((p) =>
          p._nextPaymentDateObj?.isBetween(today.startOf("month"), today.endOf("month"), null, "[]")
        );
        break;
      case "year":
        filtered = pendingPayments.filter((p) =>
          p._nextPaymentDateObj?.isBetween(today.startOf("year"), today.endOf("year"), null, "[]")
        );
        break;
      case "custom":
        if (fromDate && toDate) {
          filtered = pendingPayments.filter((p) =>
            p._nextPaymentDateObj?.isBetween(dayjs(fromDate), dayjs(toDate), null, "[]")
          );
        } else {
  ...
  // Show modal
  const handleShowModal = (payment, mode) => {
  ...
  const handleUpdatePayment = async () => {
    try {
      await axios.put(
        `https://api.rentyourpc.com/api/payment/updatepayment/${selectedPayment._id}`,
        formData
      );
    ...
    <Container fluid className="py-4">
      <Card className="p-3 shadow-sm border-0">
        <Row className="align-items-center mb-3">
          <Col>
            <h5 style={{ fontSize: "15px", fontWeight: 600 }}>Pending Payment Report</h5>
          </Col>
        </Row>
        ...
        <Row className="align-items-end mb-3" style={{ fontSize: "13px" }}>
          ...
          <Col xs="auto" className="mt-3 mt-md-0">
            <Button variant="outline-primary" size="sm" onClick={() => handleFilter("custom")}>
              Apply
            </Button>
          </Col>
          <Col xs="auto">
              ...
              {currentPayments.length > 0 ? (
                currentPayments.map((p, i) => (
                  <tr key={p._id}>
                    ...
                    <td>{p._nextPaymentDateObj ? p._nextPaymentDateObj.format("DD-MM-YYYY") : "N/A"}</td>
                    <td>{p.amount ?? 0}</td>
                     {/* <td>
  <span
    style={{
    ...
    {p.paymentStatus.toUpperCase()}
                    ...
                    <td>
                      <Button
                        ...
                        size="sm"
                        className="me-2"
                        onClick={() => handleShowModal(p, "create")}
                      >
                        Pay
                      </Button>
                      <Button
                        ...
                        size="sm"
                        className="me-2"
                        // onClick={() => navigate("generate-payment-invoice")}
                         onClick={() =>
    navigate("/generate-payment-invoice", {
      state: { orderData: p,}, 
    })
  }
                      >
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => handleShowModal(p, "update")}
                      >
                        UpdatePay
                      </Button>
                    </td>
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered>
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "14px" }}>
            {modalMode === "create" ? "Create Payment" : "Update Payment"}
          </Modal.Title>
        </Modal.Header>
          ...
          {selectedPayment && (
            <Form>
              <Form.Group className="mb-2">
                ...
                <Form.Select name="paymentMethod" value={formData.paymentMethod} onChange={handleChange} style={textStyle}>
              ...
              <Form.Group className="mb-2">
                ...
                <Form.Select name="paymentType" value={formData.paymentType} onChange={handleChange} style={textStyle}>
                  <option value="">Select</option>
              ...
              <Form.Group className="mb-2">
                ...
                <Form.Control type="number" name="amount" value={formData.amount} onChange={handleChange} style={textStyle} />
              </Form.Group>

              <Form.Group className="mb-2">
                <Form.Label>Next Payment Date</Form.Label>
                <Form.Control type="date" name="nextPaymentDate" value={formData.nextPaymentDate} onChange={handleChange} style={textStyle} />
              </Form.Group>

              <Form.Group className="mb-2">
                <Form.Label>Payment Status</Form.Label>
                <Form.Select name="paymentStatus" value={formData.paymentStatus} onChange={handleChange} style={textStyle}>
                  <option value="">Select</option>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" size="sm" onClick={modalMode === "create" ? handleCreatePayment : handleUpdatePayment}>
            {modalMode === "create" ? "Pay" : "Update"}
          </Button>
        </Modal.Footer>
      </Modal>
    </Container>

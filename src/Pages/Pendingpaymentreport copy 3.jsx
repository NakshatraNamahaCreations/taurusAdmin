...
function Pendingpaymentreport() {
  ...
  const handleFilter = (type) => {
    ...
    switch (type) {
      case "today":
        filtered = pendingPayments.filter(
          (p) => p._nextPaymentDateObj?.isSame(today, "day")
        );
        break;
      case "tomorrow":
        filtered = pendingPayments.filter(
          (p) => p._nextPaymentDateObj?.isSame(today.add(1, "day"), "day")
        );
        break;
      case "week":
  ...
  //   try {
  //     await axios.put(
  //       `https://api.rentyourpc.com/api/payment/createpayment/${selectedPayment._id}`,
  //       formData
  //     );
  ...
  // };
   ...
   const handleUpdatePayment = async () => {
    try {
      await axios.post(
        'https://api.rentyourpc.com/api/payment/createpayment',
        formData
      );
      alert("Payment updated successfully!");
      setShowModal(false);
                  ...
                  <tr key={p._id}>
                    ...
                    <td>
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => navigate(`/generate-invoice/${p.orderId._id}`)}
                      >
                        Invoice
                      </Button>
                    </td>
